$(function () {
	...
	$('[data-toggle="tooltip"]').tooltip();
  	$('.user-circle,.capacity-msg,.count-cross,.backdrop,.premier-text').hide();
	var width = $(window).width();
    $("#navbarSupportedContent").on("shown.bs.collapse", function() {
  		$('.user-circle').show();
		$('.navbar').css({"border-bottom":"none","background-color":"#292F33"});
		$('body').css("overflow","hidden");
	}).on('hidden.bs.collapse', function() {
		$('.user-circle').hide();
		$('.navbar').css({"border-bottom":"1px solid hsla(0,0%,98%,.5)","background-color":"transparent"});
	...
	$("#viewMoreday1,#viewMoreday2").on("show.bs.collapse", function() {
  		$(this).parent().find('.view-more-btn.active_hidden').hide();
	}).on('hide.bs.collapse', function() {
		$(this).parent().find('.view-more-btn.active_hidden').show();
	});		
	$('.booking-session .booking-session-list li a').on('shown.bs.tab', function (e) {
	  e.preventDefault();
	  $('.backdrop').show();
	  $('body').css("overflow","hidden");
